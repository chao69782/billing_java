...
public class AssistantTools {
    ...
    @PostConstruct
    public void init() {
        cachedCategories = categoryService.list();
    }

    /**
     * 获取所有分类的格式化字符串，用于 System Prompt
     */
    public String getCategoriesForPrompt() {
        if (cachedCategories == null) {
            cachedCategories = categoryService.list();
        }
        StringBuilder sb = new StringBuilder();
        cachedCategories.forEach(c -> sb.append(String.format("- %s (类型: %s)\n",
                c.getCategoryName(),
    ...
    public String saveRecord(
        ...
        try {
            // 匹配分类
            Category matchedCategory = matchCategory(categoryName, recordType);
            
            if (matchedCategory == null) {
                return "记账失败：找不到分类 '" + categoryName + "'，请先调用 getAllCategories 查看可用分类。";
            }
            ...
            record.setCategoryId(matchedCategory.getId());
            record.setRemark(remark);
            record.setRecordTime(recordTime);
            record.setRecordType(recordType);
            recordService.save(record);
            
            return "记账成功，已归类为：" + matchedCategory.getCategoryName();
        } catch (Exception e) {
            log.error("保存记录失败", e);
     ...
     * 内存中匹配分类
     */
    private Category matchCategory(String categoryName, Integer recordType) {
        if (cachedCategories == null) {
            cachedCategories = categoryService.list();
        }
        List<Category> categories = cachedCategories;
        Category matchedCategory = null;

        // 1. 精确匹配（名称 + 类型）
        for (Category c : categories) {
            if (c.getCategoryName().equals(categoryName) && c.getCategoryType().equals(recordType)) {
                return c;
            }
        }
        ...
        // 3. 包含匹配（模糊匹配）
        for (Category c : categories) {
            if (c.getCategoryName().contains(categoryName) || categoryName.contains(c.getCategoryName())) {
                // 如果类型也匹配，则优先返回
                if (c.getCategoryType().equals(recordType)) {
                    return c;
                }
                // 否则暂存，继续寻找是否有类型匹配的
                if (matchedCategory == null) {
